...
function drawtable() {
  chrome.storage.local.get('sendoutlist', (results) => {
    if (chrome.runtime.lastError || undefined === results.sendoutlist) {
      console.log('no sendoutlist');
    } else {
      $('#commenttable>tbody').empty();
      $('#ctcap').empty = '';
      results.sendoutlist.books.forEach((e) => {
        if (e[2].length !== 0) {
          addTableElement(e);
        }
      });
      document.getElementById('commenttable').style.display = 'table';
      const thattime = new Date(results.sendoutlist.updatetime);
      const updatetimeele = document.createElement('div');
      updatetimeele.setAttribute('id', 'lastupdate');
      const updatetime = document.createTextNode(`最後更新 : ${thattime.toLocaleDateString('zh-Hant-TW')}${thattime.toLocaleTimeString('zh-Hant-TW')}`);
      updatetimeele.appendChild(updatetime);
      const filenameele = document.createElement('div');
      filenameele.setAttribute('id', 'uploadfile');
      const filename = document.createTextNode(`檔名${results.sendoutlist.filename}`);
