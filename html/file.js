...
document.getElementById('fileConfirm-btn').addEventListener('click', fileviewerload);

function fileviewerload(event) {
  ...
  if (file) {
    ...
    reader.onload = function(evt) {
      ...
      let result = arys.data.filter((record) => record[7] === '3-安定區圖書館(BTR)' && record[8].includes('安定')).map(function(e) {
 return e[9];
});
      console.log(result);
