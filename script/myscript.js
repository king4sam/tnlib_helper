...
let host = 'http:\/\/163\.26\.71\.107\/toread\/';
...
( function( document ) {
  ...
  let manageinbatch = new RegExp( host + 'internaltranzit\/manage_in_batch*' );
  if ( manageinbatch.test( location.href ) ) {
    let AssignedReportsObserver = new MutationObserver(InputClear);
...
( function( document ) {
  chrome.storage.local.get( 'hotkeys', function( results ) {
    ...
    function keydowne( event ) {
      ...
      let cardNumberFieldaction = function() {
        if ( document.getElementById( 'cardNumberField' ) ) {
          event.preventDefault();
          document.getElementById( 'cardNumberField' ).focus();
        }
      };
      let loandeskaction = function() {
        event.preventDefault();
        window.location = '/toread/circulation/pages/loan_desk';
      };
      let TransitItemsToBesendaction = function() {
        let TransitItemsToBesend = new RegExp( host + 'circulation\/exttransit\/transit_items_to_send' );
      ...
      let managecloseaction = function() {
      ...
      let targetname = namecodemap.find( ( e ) => {
 return e.code === x;
} );
      if ( targetname ) {
        console.log( targetname.name );
        let targetaction = keyconbinations.find( ( e ) => {
 return e.name === targetname.name;
} );
        if ( targetaction ) {
...
chrome.runtime.onMessage.addListener(
  function( request, sender, sendResponse ) {
    ...
    if ( manageinbatch.test( location.href ) ) {
      let seletcObserver;
      let setselect = function( cursta, proper ) {
      ...
      let obtarget = document.getElementById( 'HiddenBrowse' );
...
( function( document ) {
  let TransitItemsToBeReceived = new RegExp( host + 'circulation\/exttransit\/TransitItemsToBeReceived,popupComponent.*' );
...
( function( document ) {
  ...
  let exttransiturl = new RegExp( host + 'circulation\/exttransit\/required_from_ext_transit*' );
  if ( exttransiturl.test( location.href ) && document.getElementById( 'AssignedReports' ) !== null ) {
    let AssignedReportsObserver = new MutationObserver( AssignedReportsSetDefault );
...
( function( document ) {
  ...
  let exttransiturl = new RegExp( host + 'circulation\/pages\/search_transactions*' );
  if ( exttransiturl.test( location.href ) && document.getElementById( 'AssignedReports' ) !== null ) {
    let AssignedReportsObserver = new MutationObserver( AssignedReportsSetDefault );
    AssignedReportsObserver.observe( document.getElementById( 'AssignedReports' ), {'subtree': true, 'childList': true, 'characterData': true} );
  }
...
( function( document, location, $ ) {
  let loandeskurl = new RegExp( host + 'circulation\/pages\/loan_desk*' );
  if ( loandeskurl.test( location.href ) ) {
    document.borrowcount = function( e ) {
      // fetch records of book
      let a = $( '#If_43 tr' ).toArray();
      a.shift();
      ...
      function istoday( e ) {
        let t = new Date( e.children[9].innerText );
    ...
    let addborrowcount = function( records ) {
      let borrowcountposition = '#viewPatronDetailsComponent> tbody> tr> td >table >tbody> tr> td >table >tbody:last-child';
      let reader = $( borrowcountposition );
      if ( reader.length !== 0 ) {
        reader.append(
    ...
    let modifyborrowcount = function( records ) {
      $( '#todayborrowcount font' ).innerText = document.borrowcount();
    };

    // listen to panel change
    let PatronObserver = new MutationObserver( addborrowcount );
    let TransactionsObserver = new MutationObserver( modifyborrowcount );
...
( function( document, $ ) {
  console.log( 'hi' );
  let modifyprintview = function( records ) {
    console.log( 'yo' );
  ...
  let PrintContentObserver = new MutationObserver( modifyprintview );
  let obtarget = document.getElementById( 'TransactionsContent' ) || document.getElementById( 'TransactionsDesk' );
  if ( obtarget !== null ) {
